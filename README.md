...
This standalone viewer remains useful for development, manual CSV exploration, and very large CSV files. The built-in `paping-go report` command creates a self-contained HTML report by embedding all CSV rows in the chart, which is easy to share but can produce large HTML files for long-running measurements. The standalone viewer loads CSV data locally in the browser instead of embedding it into a generated report.
